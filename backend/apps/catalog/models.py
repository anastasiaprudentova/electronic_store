...
class Product(TimeStampedModel):
    ...
    def get_main_image_url(self, request=None):
        """Возвращает URL главного изображения"""
        main_gallery = self.gallery.filter(is_main=True).first()
...
class AttributeValue(models.Model):
    ...
    text_value = models.CharField(
        "Текстовое значение", max_length=255, blank=True, null=True
    )
...
class Stock(models.Model):
    ...
    warehouse_id = models.IntegerField(
        "ID склада", default=1, validators=[MinValueValidator(1)]
    )
    quantity = models.IntegerField(
        "Количество", default=0, validators=[MinValueValidator(0)]
    )
    reserved = models.IntegerField(
        "Зарезервировано", default=0, validators=[MinValueValidator(0)]
    )
...
class ProductImage(TimeStampedModel):
    ...
    image = models.ImageField(
        ...
        help_text="Рекомендуемый размер: 1200x1200 px",
    )
    alt_text = models.CharField("Альтернативный текст", max_length=255, blank=True)
    order = models.PositiveIntegerField(
        "Порядок", default=0, help_text="Чем меньше число, тем выше изображение"
    )
    is_main = models.BooleanField(
        "Главное изображение",
        default=False,
        help_text="Будет использоваться как основное фото товара",
    )
